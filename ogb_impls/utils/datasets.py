...
class GCDataset:
    ...
    def compute_success(self, state_rep, goal_rep):
        assert state_rep.shape == goal_rep.shape
        diff = state_rep - goal_rep
        if "maze" in self.env_name:
            # --- extract xy difference ---
            diff = diff[..., :2]
            goal_tol = 0.5  # goal_tol = 1.0 if "point" in self.env_name else 0.5
            return np.linalg.norm(diff, axis=-1) <= goal_tol
        elif "cube" in self.env_name:
            env = getattr(self.env, "unwrapped", self.env)
            num_cubes = getattr(env, "_num_cubes", None)
            diff_dim = diff.shape[-1]
            # --- state observations: each cube contributes 9 dimensions i.e. [3 positions, 4 quaternions, 2 yaw angles] ---
            cube_chunk = 9 * num_cubes
            if diff_dim >= cube_chunk:
                # --- keep only xyz positions for each cube ---
                diff = diff[..., -cube_chunk:].reshape(*diff.shape[:-1], num_cubes, 9)[
                    ..., :3
                ]
            return np.all(np.linalg.norm(diff, axis=-1) <= 0.4, axis=-1)
        elif "puzzle" in self.env_name:
            env = getattr(self.env, "unwrapped", self.env)
            num_buttons = getattr(env, "_num_buttons", None)
            diff_dim = diff.shape[-1]
            # --- state observations: each button contributes 4 dimensions i.e. [2 one-hot state, 1 pos, 1 vel] ---
            button_chunk = 4 * num_buttons
            if diff_dim >= button_chunk:
                # --- keep only the 2D one-hot state vectors for each button ---
                diff = diff[..., -button_chunk:].reshape(
                    *diff.shape[:-1], num_buttons, 4
                )[..., :2]
            # If the one-hot vectors match (e.g. [1,0] vs [1,0]), the diff norm is 0.
            # If they mismatch (e.g. [1,0] vs [0,1]), the diff is [1,-1] and the norm is sqrt(2) ~ 1.414.
            # Checking if the norm is < 0.5 safely guarantees we strictly matched the correct states.
            return np.all(np.linalg.norm(diff, axis=-1) < 0.5, axis=-1)
        else:
            return False
    ...
    def sample(self, batch_size: int, idxs=None, evaluation=False):
        ...
        successes = np.logical_or(
            (idxs == value_goal_idxs),
            np.logical_or(
                self.compute_success(
                    batch["observations"], batch["value_goal_observations"]
                ),
                self.compute_success(
                    batch["next_observations"], batch["value_goal_observations"]
                ),
            ),
        ).astype(float)
...
class HGCDataset(GCDataset):
    ...
    def sample(self, batch_size: int, idxs=None, evaluation=False):
        ...
        high_value_successes = np.logical_or(
            (idxs == high_value_goal_idxs),
            np.logical_or(
                self.compute_success(batch["observations"], batch["high_value_goals"]),
                self.compute_success(
        ...
        batch["high_value_rewards"] = high_value_successes - (
        ...
        if "low_discount" in self.config:
            low_value_goal_idxs = self.sample_goals(
                idxs,
                self.config["low_value_p_curgoal"],
                self.config["low_value_p_trajgoal"],
                self.config["low_value_p_randomgoal"],
                self.config["low_value_geom_sample"],
            )
            batch["low_value_goals"] = self.get_observations(low_value_goal_idxs)
            low_value_successes = np.logical_or(
                (idxs == low_value_goal_idxs),
                np.logical_or(
                    self.compute_success(
                        batch["observations"], batch["low_value_goals"]
                    ),
                    self.compute_success(
            ...
            batch["low_value_rewards"] = low_value_successes - (
                1.0 if self.config["gc_negative"] else 0.0
            )
        ...
        high_actor_next_idxs, _ = self.compute_high_next_idxs(
            idxs,
            final_state_idxs,
