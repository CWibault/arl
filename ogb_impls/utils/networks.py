...
def length_normalize(x):
    return x / jnp.maximum(jnp.linalg.norm(x, axis=-1, keepdims=True), 1e-8) * jnp.sqrt(x.shape[-1])
...
def soft_length_normalize(x):
    
    sq_norm = jnp.sum(jnp.square(x), axis=-1, keepdims=True)
    # --- prevent nan gradients from sqrt(0) ---
    safe_norm = jnp.sqrt(jnp.maximum(sq_norm, 1e-12))
        
    # --- use Taylor expansion for small norms to prevent n^3 division explosion ---
    scale = jnp.where(
        safe_norm < 1e-2,
        1.0 - sq_norm / 3.0, 
        jnp.tanh(safe_norm) / safe_norm
    )
        
    return x * scale * jnp.sqrt(x.shape[-1])

class SoftLengthNormalize(nn.Module):
    """
    Numerically stable Soft length normalization layer.
    Normalizes the input along the last dimension to have a length of sqrt(dim) with a soft thresholding.
    Aim is output = x * tanh(||x||) / ||x|| ~ x when x is small, and ~ x / ||x|| when x is large.
    """
...
class HGCEncoder(nn.Module):
    """Hierarchical goal-conditioned encoder.

    This module both encodes and decodes observations and goals into a latent representation.
    Its point is to push together observations and goals with similar optimal actions. 

    Attributes:
        hidden_dims: Hidden layer dimensions.
        latent_dim: Latent dimension.
        layer_norm: Whether to apply layer normalization.
        final_layer: Final layer to apply to the encoded representation.
    """
    hidden_dims: Sequence[int]
    latent_dim: int
    layer_norm: bool = True
    final_obs_layer: str = 'none'
    final_goal_layer: str = 'soft_length_normalize'

    def setup(self):
        if self.final_obs_layer == 'soft_length_normalize':
            final_obs_layer = SoftLengthNormalize()
        elif self.final_obs_layer == 'length_normalize':
            final_obs_layer = LengthNormalize()
        elif self.final_obs_layer == 'tanh':
            final_obs_layer = nn.tanh
        elif self.final_obs_layer == 'none':
            final_obs_layer = Identity()
        else:
            raise ValueError(f'Invalid final layer: {self.final_obs_layer}')
        
        self.obs_encoder = nn.Sequential([
            MLP((*self.hidden_dims, self.latent_dim), activate_final=False, layer_norm=self.layer_norm),
            final_obs_layer,
            ])

        if self.final_goal_layer == 'soft_length_normalize':
            final_goal_layer = SoftLengthNormalize()
        elif self.final_goal_layer == 'length_normalize':
            final_goal_layer = LengthNormalize()
        elif self.final_goal_layer == 'tanh':
            final_goal_layer = nn.tanh
        elif self.final_goal_layer == 'none':
            final_goal_layer = Identity()
        else:
            raise ValueError(f'Invalid final layer: {self.final_obs_layer}')
    ...
    def __call__(
        ...
        observations=None, 
        goals=None):
        """Return either a probability distribution over actions or the encoded observation and goal representation.

        Args:
            observations: Observations.
            goals: Goals.
        """
        if observations is not None and goals is not None:
            return jnp.concatenate([self.obs_encoder(observations), self.goal_encoder(goals)], axis=-1)
        elif observations is not None:
            return self.obs_encoder(observations)
        elif goals is not None:
            return self.goal_encoder(goals)
        else:
            raise ValueError('Either observations or goals must be provided.')
...
class ActorVectorField(nn.Module):
    ...
    @nn.compact
    def __call__(self, observations, goals, actions, times=None):
...
class Decoder(nn.Module):
    ...
    def setup(self):
        self.decoder_net = MLP(self.hidden_dims, activate_final=True)
        self.mean_net = nn.Dense(self.output_dim, kernel_init=default_init(self.final_fc_init_scale))
        if self.state_dependent_std:
            self.log_std_net = nn.Dense(self.output_dim, kernel_init=default_init(self.final_fc_init_scale))
        else:
            if not self.const_std:
                self.log_stds = self.param('log_stds', nn.initializers.zeros, (self.output_dim,))
    ...
    def __call__(
        ...
        Args:
            ...
            temperature: Scaling factor for the standard deviation.
        """
        outputs = self.decoder_net(embeddings)
        means = self.mean_net(outputs)
